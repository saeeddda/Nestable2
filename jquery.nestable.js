    ...
    Plugin.prototype = {
        ...
        asNestedSet: function () {
            ...
            function isInt(value) {
                return !isNaN(value) && Number.isFinite(Number(value)) && Math.floor(value) === value;
            }
        },
        ...
        restoreItemAtIndex: function (dragElement, indexArray) {
            ...
            function placeElement(currentEl, dragElement) {
                if (indexArray[lastIndex] === 0) {
                    $(currentEl).prepend(dragElement.clone(true));
                }
                else {
                    $(currentEl.children[indexArray[lastIndex] - 1]).after(dragElement.clone(true));
                }
            }
        ...
        dragMove: function (e) {
            ...
            if (opt.scroll) {
                ...
                // Check if scrollParent exists (jQuery UI)
                if (typeof $.fn.scrollParent !== 'undefined') {
                    ...
                    // Fallback to window scrolling if scrollParent not available
                    if (e.pageY - $document.scrollTop() < opt.scrollSensitivity)
                        scrolled = $document.scrollTop($document.scrollTop() - opt.scrollSpeed);
                    else if ($window.height() - (e.pageY - $document.scrollTop()) < opt.scrollSensitivity)
                        scrolled = $document.scrollTop($document.scrollTop() + opt.scrollSpeed);
